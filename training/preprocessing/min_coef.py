...
class MinCoef(CoefficientInf):
    """
    Normalization strategy that uses the minimum value of a cycle as the offset.
    ...
    minimum value, effectively shifting the baseline to zero before scaling. 
    If a cycle has no variance (max == min), it falls back to a global 
    uniform scaling.
    """
    ...
    def get_coefficient(self, stat_list: List[Optional[DataPoint]]) -> List[Optional[DataPoint]]:
        ...
        The offset is set to the cycle's minimum value, and the scale is 
        ...
        for index, stat in enumerate(stat_list):
            if stat:
                ...
                scale_offset[OFFSET] = stat.data[MIN_INDEX]
