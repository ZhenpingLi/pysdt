...
def is_overlap(event : SDTEventData, start_time: float, end_time: float) -> bool:
    """Checks if the event segment overlaps with a given time range."""
    return event.event_start <= end_time and start_time <= event.event_end

def get_oc_value(event_data_list: List[SDTEventData], start: float, end: float, state : str = DEFAULT) -> float:
    """
    Calculates the aggregate outlier metric value for a specific period and state.

    Args:
        event_data_list (List[SDTEventData]): The list of processed events.
        start (float): The start of the time range.
        end (float): The end of the time range.
        state (str): Optional filter for the operational state.

    Returns:
        float: The summed outlier value.
    """
    if not event_data_list:
        return 0.0

    total_event_value = 0.0
    for event in event_data_list:
        # Check state index if provided
        if state is not None and event.event_state != state:
            continue

        # Check for time overlap if start and end are provided
        if start is not None and end is not None:
            if is_overlap(event, start, end):
                total_event_value += event.event_value
        else: # If no time range, sum all
            total_event_value += event.event_value

    return float(total_event_value)
